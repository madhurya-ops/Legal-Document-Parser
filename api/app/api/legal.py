...
from app.schemas import (
    ...
    DocumentAnalysisCreate, DocumentAnalysisResponse, AnalysisType
)
    ...
    try:
        ...
        if request.document_id:
            ...
            if document:
                ...
                await crud.create_document_analysis(db, analysis_data)
...
async def search_precedents(
    ...
    try:
        ...
        result = await precedent_engine.find_relevant_precedents(
            query=request.query,
            jurisdiction=request.jurisdiction,
...
@router.get("/documents/{document_id}/analyses/{analysis_type}")
async def get_document_analysis_by_type(
    ...
    try:
        analysis = await crud.get_analysis_by_type(db, document_id, analysis_type.value, str(current_user.id))
        if not analysis:
